...
my_tibble <- tibble(x = 1:100)

even_nums <- subset(my_tibble, x %% 2 == 0)
...
* summarise(new_variable = a_function(another_variable))  # SUMMARISE TAKES COMMON SUMMARY FUNCTIONS LIKE:
*                                                         # MEAN(), MEDIAN(), ETC.
```
...
The following code divides our weather data frame by 'YEAR' and then calculates the correlation between temperature and relative humidity for each yeaer
